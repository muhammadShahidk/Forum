...
import { ReplyRequestDto } from '../Modals/Dtos/ReplyRequestDto';

@Injectable({
  providedIn: 'root',
})
export class RepliesService {
  async getCommentsReply(commentId:number): Promise<ReplyResponseDto[]> {
    const responses = await this.api.handleRequest(
      this.http.get<apiResponce<ReplyResponseDto[]>>(
        RouteCategories.User.Replies.GET(commentId)
      )
    );
    return responses.Data.sort((a, b) => {
      return Number(new Date(b.dateCreateAt)) - Number(new Date(a.dateCreateAt));
    });
  }
  ...
  async addReply(commentId:number , Reply:ReplyRequestDto): Promise<any> {
    return await this.api.handleRequest(
      this.http.post(RouteCategories.User.Replies.POST(commentId), Reply)
    );
  }
